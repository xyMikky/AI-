...
description: Amazon 多 ASIN 竞品分析数据对齐审计工具。用于 amazon-competitor-analyzer 完成多 SKU 输出后，检查每个 ASIN 子目录中的 metadata、report.json 图片报告、Markdown、visual.json、HTML、评论聚合、评论证据和评论看板是否全部指向同一目标 ASIN，并检查单品 HTML 是否出现空套图 src，防止多 ASIN 批处理时发生目录串号、评论样本串号、图片报告缺失、看板混入其它变体 ASIN或报告引用错位。适用于“核对数据是否错位”“多 ASIN 分析数据校验”“套图无法查看”“评论数据有没有串 ASIN”“批量竞品报告交付前审计”等场景。关键词：多ASIN核对、ASIN对齐、数据错位、图片链路、report.json、gallery-main、评论串号、竞品分析校验、review dashboard、metadata、visual.json、review_evidence、asin alignment。
---
...
这个 Skill 是 `amazon-competitor-analyzer` 的交付后审计工具，专门解决多 ASIN 批量竞品分析最容易出错的问题：**一个 ASIN 的报告、元数据、评论样本、图片报告或看板误用了另一个 ASIN 的数据**，以及**图片文件存在但单品 HTML 因 `report.json` 缺失而无法显示套图/A+**。
...
4. **图片链路对齐**
   - 若 `gallery/` 或 `aplus/` 有图片，则 `report.json` 必须存在。
   - `report.json.gallery.files[]` 和 `report.json.aplus.groups[].files[]` 引用的图片文件必须真实存在。
   - `metadata.media.gallery_count`、`metadata.media.aplus_module_count` 在对应图片存在时必须非空。
   - `index.html` 不得出现 `class="gallery-main" src=""`。
   - `index.html` 必须引用 `report.json` 中的首张套图和首张 A+ 图，防止左侧套图栏空白。
...
python ".codex/skills/amazon-multi-asin-data-auditor/scripts/audit_asin_alignment.py" `
...
- `report.json` 图片报告、`metadata.media` 和 `index.html` 图片引用一致；不存在空套图 `src=""`
...
| `image_report_json_exists_when_assets_exist` FAIL | 图片目录有素材但缺少 `report.json`，HTML 渲染器拿不到图片列表 | 重跑 `amazon-image-extractor`；若图片完整只丢了报告，可从 `gallery/` / `aplus/` 合成最小 `report.json` 后重渲染 |
| `html_gallery_src_not_empty` FAIL | 单品 HTML 主套图 `src=""`，页面无法查看套图 | 补齐 `report.json` 和 `metadata.media`，重新运行 `render_report_html.py` |
| `html_references_report_gallery_asset` / `html_references_report_aplus_asset` FAIL | HTML 没有引用 `report.json` 里的图片 | 重渲染单品 `index.html`，不要只改图片目录 |
