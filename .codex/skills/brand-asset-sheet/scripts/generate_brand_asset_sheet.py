...
项目根目录推算规则（优先级从高到低）：
  ...
  2. 从 __file__ 向上推算5层：scripts/ → brand-asset-sheet/ → skills/ → .codex/ → 项目根
  3. 当前工作目录（cwd）兜底
"""
...
def main():
    ...
    if args.project_root.strip():
        ...
        try:
            # 脚本位置：<project_root>/.codex/skills/brand-asset-sheet/scripts/此文件
            # 向上5层：scripts → brand-asset-sheet → skills → .codex → 项目根
