...
用法：
    ...
    # 方式 3：从文件读取
    python create_skill.py --name my-skill --file skill.md
...
class SkillCreator:
    ...
    def __init__(self, base_dir: str = None):
        ...
        Args:
            base_dir: 基础目录（默认为 .codex/skills）
        """
        if base_dir is None:
            # 自动查找工作区根目录的 .codex/skills
    ...
    def _find_base_dir(self) -> Path:
        ...
        从当前目录向上查找，直到找到 .codex/skills 目录
        """
        current = Path.cwd()
        
        # 最多向上查找 5 层
        for _ in range(5):
            codex_skills = current / ".codex" / "skills"
    ...
    def validate_content(self, content: str) -> Tuple[bool, str]:
        ...
        if desc_match:
            ...
            if chinese_chars > total_chars * 0.3:
                if total_chars < 40:
                    return False, f"description 太短（当前 {total_chars} 字，建议至少 40 字，并写清触发场景）"
                if total_chars > 1200:
                    return False, f"description 太长（当前 {total_chars} 字，建议拆分细节到正文或 references）"
    ...
    def create_skill(self, skill_name: str, content: str, 
        ...
        Args:
            ...
            force: 是否强制覆盖已存在的文件
            create_dirs: 要创建的可选目录列表 ['agents', 'scripts', 'references', 'assets']。
                兼容旧参数 'templates'，会映射为 'assets/templates'。
        ...
        try:
            ...
            created_dirs = []
            if create_dirs:
                valid_dirs = ['agents', 'scripts', 'references', 'assets']
                for dir_name in create_dirs:
                    dir_name = dir_name.strip()
                    if dir_name == 'templates':
                        template_dir = skill_dir / 'assets' / 'templates'
                        template_dir.mkdir(parents=True, exist_ok=True)
                        created_dirs.append('assets/templates')
                    elif dir_name in valid_dirs:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='创建 Codex Agent Skill 文件',
...
示例:
  ...
  python create_skill.py --name my-skill --content "..." --force
    ...
    # 其他选项
    parser.add_argument('--base-dir', type=str, help='基础目录（默认为 .codex/skills）')
    parser.add_argument('--force', action='store_true', help='强制覆盖已存在的文件')
    parser.add_argument('--dirs', type=str, help='创建可选目录（逗号分隔）：agents,scripts,references,assets；旧 templates 会映射到 assets/templates')
