...
Usage:
  ...
  python scripts/response_io.py run --script <PATH> --out-dir <DIR> --params-file <params.json> [--label NAME] [--timeout SEC]
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if args.params and args.params_file:
        _err("run: pass either positional JSON params or --params-file, not both")
    if not args.params and not args.params_file:
        _err("run: JSON params are required; pass positional params or --params-file")
    ...
    if args.params_file:
        ...
        if not params_path.is_file():
            _err(f"--params-file path not found: {params_path}")
        try:
            params_text = params_path.read_text(encoding="utf-8-sig")
        except OSError as e:
            _err(f"Failed to read --params-file {params_path}: {e}")
    else:
        params_text = args.params

    # Validate before creating an output file. This prevents bad PowerShell
    # quoting from leaving empty files that later glob patterns may mistake for
    # successful API responses.
    try:
        json.loads(params_text)
    except json.JSONDecodeError as e:
        _err(f"Invalid parameter JSON before API call: {e}")
    ...
    try:
        proc = subprocess.run(
            [sys.executable, str(main_script), params_text],
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    # Write stdout to disk. Failed calls are renamed to *.failed.json so normal
    # success globs such as *reviews_linkfox_raw.json never pick them up.
    try:
    ...
    if returncode != 0 or timed_out:
        failed_file = out_file.with_name(f"{out_file.stem}.failed{out_file.suffix}")
        try:
            out_file.replace(failed_file)
            out_file = failed_file
        except OSError:
            pass
...
def main() -> int:
    ...
    p_run.add_argument("params", nargs="?", help="JSON params string passed verbatim to the main script (argv[1]).")
    p_run.add_argument("--params-file", default=None, help="UTF-8 JSON file whose contents are passed to the main script. Prefer this on Windows/PowerShell.")
