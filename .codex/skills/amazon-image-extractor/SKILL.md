...
> ⚠️ **下游渲染契约**：`report.json` 不是可选日志，而是 `amazon-competitor-analyzer/scripts/render_report_html.py` 左侧套图栏和 A+ 预览的数据源。渲染器不自动扫描 `gallery/` / `aplus/` 目录；如果 `report.json` 缺失或 `gallery.files[] / aplus.groups[].files[]` 为空，即使图片文件实际存在，单品 HTML 也会出现 `class="gallery-main" src=""`，用户看到的就是“套图无法查看”。

提取完成后必须验收：

- `report.json` 存在，且 `gallery.files[]` / `aplus.groups[].files[]` 使用相对路径（如 `gallery/01_xxx.jpg`、`aplus/g01_01_xxx.jpg`）。
- `report.json` 引用的每个图片文件都在输出目录下真实存在。
- 若输出目录已有 `gallery/` 或 `aplus/` 图片，但 `report.json` 不存在或为空，必须重跑提取；只有图片已完整、且确认只是 `report.json` 丢失时，才允许从资产目录合成最小 `report.json` 给下游渲染。
- 交给 `amazon-competitor-analyzer` 前，同步让 `metadata.media.gallery_count / aplus_module_count / aplus_image_count` 使用 `report.json` 的计数，避免基础信息卡显示 `None / None`。
...
- `.codex/skills/amazon-image-extractor/scripts/extract_and_stitch.py` — 唯一入口脚本
- `.codex/skills/amazon-image-extractor/scripts/requirements.txt` — Python 依赖（首次使用前执行 `pip install -r .codex/skills/amazon-image-extractor/scripts/requirements.txt`）

> V2.4 起本 Skill 的脚本由 `工具/amazon_image_extractor/` 迁移到此目录，遵循 Skill 自包含原则（详见 `.codex/rules/skill-file-structure.mdc`）。原 `工具/amazon_image_extractor/` 已下线。
