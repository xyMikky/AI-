...
```bash
# 最简：输出到与源文件同名的 .json
python .cursor/skills/excel-to-json/scripts/excel_to_json.py --input "d:\path\data.xlsx"

# 指定输出路径与缩进
python .cursor/skills/excel-to-json/scripts/excel_to_json.py -i data.xlsx -o out.json --indent 4

# 只转换某个工作表，表头在第 2 行
python .cursor/skills/excel-to-json/scripts/excel_to_json.py -i data.xlsx --sheet Sheet1 --header-row 2

# 按列输出（列字典）而非对象数组
python .cursor/skills/excel-to-json/scripts/excel_to_json.py -i data.xlsx --orient columns

# 多工作表但只取第一个（不包工作表名层级）
python .cursor/skills/excel-to-json/scripts/excel_to_json.py -i data.xlsx --first-sheet

# 自定义行号字段名为 序号
python .cursor/skills/excel-to-json/scripts/excel_to_json.py -i data.xlsx --number-field 序号

# 不要行号字段
python .cursor/skills/excel-to-json/scripts/excel_to_json.py -i data.xlsx --no-number
```
