...
当用户只说“下载评论 / 获取评论 / 拉取评论”，且没有指定更小样本、特定星级或正负面筛选时，默认按接口最大窗口获取：
...
这样最多返回 500 条评论（每个星级桶最多 100 条，实际条数取决于该 ASIN 每个星级下可用评论数量）。

不要把 `allStarsNum=100` 用作默认下载策略；它只会返回最多 100 条混合星级样本，适合用户明确要求“小样本/抽样”时使用。

不要把 `formatType="current_format"` 用作默认下载策略；它只返回当前变体/当前颜色评论，可能少于 Amazon 前台“所有格式”视图中显示的评论数。默认最大化下载必须使用 `formatType="all_formats"`。

解析 `all_formats` 响应时要注意：评论对象内的 `asin` 可能是变体子 ASIN（例如不同颜色），不一定等于请求 ASIN。不要因此过滤掉这些评论。
