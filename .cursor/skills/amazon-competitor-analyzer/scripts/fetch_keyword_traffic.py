...
# scripts → <skill> → skills → .cursor → 工作区根目录（4 级）
ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
SKILLS = os.path.join(ROOT, ".cursor", "skills")
KW_SCRIPT = os.path.join(SKILLS, "linkfox-sif-asin-keywords", "scripts", "sif_asin_keywords.py")
SUM_SCRIPT = os.path.join(SKILLS, "linkfox-sif-asin-summary", "scripts", "sif_asin_summary.py")
...
def main():
    ...
    if args.compare_dir:
        ...
        for a in [x.strip() for x in args.asins.split(",") if x.strip()]:
            targets.append((a, args.outdir or "."))
    elif args.asin:
