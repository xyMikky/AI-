...
② 元数据抽取（必）       → .cursor/skills/amazon-competitor-analyzer/scripts/extract_metadata.py
②.4 评论数据来源决策（必）→ 用户文件优先；若只有 URL 则调用 linkfox-amazon-reviews 自动抓取
                          输出 reviews_source.json / reviews_linkfox_raw.json（按实际来源）
②.5 评论聚合（条件）     → .cursor/skills/amazon-competitor-analyzer/scripts/aggregate_reviews_json.py
                          ⚠️ 当存在用户评论文件或 ②.4 自动抓取成功时触发；默认只分析 URL 对应的目标 ASIN
                          ⚠️ V3 起数据驱动关键词/主题（不再绑定品类）+ 支持 .json/.xlsx + LinkFox data[] JSON
②.6 交互看板生成（条件） → .cursor/skills/amazon-competitor-analyzer/scripts/build_review_dashboard.py
                          ⚠️ 当存在用户评论文件或 ②.4 自动抓取成功时触发，产出第 4 件套 review_dashboard.html
                          默认只分析目标 ASIN + 离线自包含（内联 ECharts）+ 多维图表 + 时间范围框选联动
②.7 评论证据提取（条件） → .cursor/skills/amazon-competitor-analyzer/scripts/extract_review_evidence.py
                          ⚠️ 只要有评论来源就必须执行；报告优缺点必须基于全样本命中证据
②.8 关键词架构与流量入口（条件）→ .cursor/skills/amazon-competitor-analyzer/scripts/fetch_keyword_traffic.py
                          ⚠️ 当配置了 LINKFOXAGENT_API_KEY 时触发；调 SIF 反查 ASIN 流量词 + 流量来源构成
                          产出 keyword_arch.json + traffic_source.json，喂报告「三-3 关键词架构 / 三-4 流量入口」两章
...
THEN：⑥ 交接询问（强制 AskQuestion · 详见 §5.4）
   询问用户三选一：
...
> **不同品类参数差异极大，脚本不预设字段**；某产品确实没有参数表时 `specs` 为空数组 `[]`，
> 此时下游报告会自动省略「产品参数」子区，**绝不臆造参数**。
...
```powershell
# 美国站：默认按 1-5 星每档拉满，覆盖所有变体/格式，最多 500 条
$env:PYTHONIOENCODING="utf-8"
python ".cursor/skills/linkfox-amazon-reviews/scripts/response_io.py" run `
  ...
  '{"asin":"<metadata.asin>","marketplace":"US","star1Num":100,"star2Num":100,"star3Num":100,"star4Num":100,"star5Num":100,"allStarsNum":0,"sortBy":"recent","formatType":"all_formats"}'

# 非美国站：按星级尽量拉满，最多 500 条
$env:PYTHONIOENCODING="utf-8"
python ".cursor/skills/linkfox-amazon-reviews/scripts/response_io.py" run `
  --script ".cursor/skills/linkfox-amazon-reviews/scripts/amazon_reviews.py" `
  --out-dir "<阶段①输出目录>" `
  --label "reviews_linkfox_raw" `
  '{"asin":"<metadata.asin>","domainCode":"<站点代码>","star1Num":100,"star2Num":100,"star3Num":100,"star4Num":100,"star5Num":100,"sortBy":"recent","reviewerType":"all_reviews","mediaType":"all_contents","formatType":"all_formats"}'
```

`response_io.py run` 会输出落盘文件路径；将该文件路径作为后续 `<评论数据路径>`。若直接调用主脚本，也必须把完整 stdout JSON 写入 `<阶段①输出目录>/reviews_linkfox_raw.json`，不能只读取预览摘要。
...
> ⚠️ **PowerShell JSON 参数撕裂 → subprocess 兜底（V3.1 · 高频踩坑）**：上面那串内联 JSON 参数,PowerShell 有时会把引号吞掉/错误转义,报 `SyntaxError` 或 `unrecognized arguments`。一旦命中,**不要硬调引号**,改用 Python `subprocess` 以"参数列表"方式传入,彻底绕过 shell 解析:
>
> ```python
> import subprocess, sys, json
> params = {"asin":"<metadata.asin>","marketplace":"US",
>           "star1Num":100,"star2Num":100,"star3Num":100,"star4Num":100,"star5Num":100,
>           "allStarsNum":0,"sortBy":"recent","formatType":"all_formats"}
> cmd = [sys.executable,
>        ".cursor/skills/linkfox-amazon-reviews/scripts/response_io.py","run",
>        "--script",".cursor/skills/linkfox-amazon-reviews/scripts/amazon_us_reviews.py",
>        "--out-dir","<阶段①输出目录>","--label","reviews_linkfox_raw",
>        json.dumps(params)]
...
> ```
>
> **同理适用于任何"复杂 JSON / 嵌套引号"内联进 PowerShell 的场景**（如临时检查脚本）：宁可写成临时 `.py` 文件再 `python xxx.py` 执行,也不要把带引号的逻辑塞进 `python -c "..."`。
...
```powershell
$env:PYTHONIOENCODING="utf-8"
python ".cursor/skills/amazon-competitor-analyzer/scripts/build_review_dashboard.py" `
  --input "<评论数据路径 .json/.xlsx；或 reviews_linkfox_raw.json>" `
  --meta "<阶段①输出目录>/metadata.json" `
  --output "<阶段①输出目录>" `
  --aspects "<阶段①输出目录>/aspects.json" `   # 强烈建议：智能问题归类表（见下）
  --keywords "<阶段①输出目录>/keywords.json"   # 强烈建议：agent 二次校正的策展词云（见下）
# 默认只分析 --meta 里的目标 ASIN（URL 对应竞品）
# 可选：--asin <显式指定ASIN> / --all-variants <分析整个系列> / --themes <主题配置 JSON>
```

**强烈建议同时产出 `aspects.json`（智能问题归类表）**：光给关键词云不够——用户还得回去翻评论才知道"电池"到底在说续航差、易损坏还是要备用。你（AI）应**先读评论样本，把同类问题归类成「维度 → 子问题 → 关键词」三层结构**，写成 `aspects.json` 一并喂入。看板据此渲染「🧩 智能问题归类」面板：每个子问题显示命中量 + 低星比色条 + **一句定位到关键词上下文的代表性原话**，点击子问题即把下方「评论明细」筛成该问题的全部原话（下钻看"为什么"）。
...
#### 策展词云 `keywords.json`（强烈建议，agent 二次校正）
...
```powershell
$env:PYTHONIOENCODING="utf-8"
python ".cursor/skills/amazon-competitor-analyzer/scripts/extract_review_evidence.py" `
  --input "<评论数据路径 .json/.xlsx；或 reviews_linkfox_raw.json>" `
  --meta "<阶段①输出目录>/metadata.json" `
...
### 阶段 ②.8：关键词架构与流量入口采集（条件触发）

**触发条件**：环境配置了 `LINKFOXAGENT_API_KEY`（与评论抓取同一把钥匙）。缺失则跳过、不阻塞三件套。
...
$env:PYTHONIOENCODING="utf-8"
# 单 ASIN（单 URL 流程）
python ".cursor/skills/amazon-competitor-analyzer/scripts/fetch_keyword_traffic.py" `
...
报告消费：把 `keyword_arch.json` → visual.json 的 `keyword-arch` section（章节「三-3」）；`traffic_source.json` → `traffic-entry` section（章节「三-4」）。两章为**条件可选模块**，置于 `info-layers`（三-2）之后、`two-column-list`（四）之前，不进入收尾四联与 V4 客户洞察块。
...
> 工作量预算（仅参考）：单个 SKU 的阶段 ⑤ 约消耗 8-15 分钟 token 预算（md ~1500 字 + visual.json ~400 行 + 渲染校验）。N 个 SKU 直接乘以 N。**如果预算紧张，正确做法是 `AskQuestion` 与用户拆批次，而不是擅自降级为口头评分**。
...
```powershell
python ".cursor/skills/amazon-competitor-analyzer/scripts/validate_report_consistency.py" `
  --dir "<阶段①输出目录>"
```
...
**信息层级**：
- 第一层 · 主体写真 / 视觉风格 → {{module_layer_1}}
- 第二层 · 颜色选择 / 穿搭展示 → {{module_layer_2}}
- 第三层 · 尺码 / 买家秀 / 详细信息 → {{module_layer_3}}
...
```powershell
python ".cursor/skills/amazon-competitor-analyzer/scripts/render_report_html.py" `
...
| 3   | 二 · 风格五标签 · 视觉摘要         | `tag-cards`        | 强制                     |
| 4   | 三-1 · 卖点排序 · 三层递进        | `step-ladder`      | 强制                     |
| 5   | 三-2 · 信息层级 · A+ 分层拆解     | `info-layers`      | 强制（A+ 模块 ≥ 3 时）        |
...
| type                        | 用途                                                                                                                                                                                                                                         | 关键字段                                                                                                                                                              |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tag-cards`                 | 风格摘要（胶囊 + 色卡 + 描述）                                                                                                                                                                                                                         | `items[{name, pills[], swatches[], desc}]`                                                                                                                        |
...
| `quote`                     | 高亮引用块                                                                                                                                                                                                                                      | `text`                                                                                                                                                            |
| `step-ladder`               | 编号阶梯卡（卖点排序 / 步骤）                                                                                                                                                                                                                           | `items[{name, desc, badge?}]`                                                                                                                                     |
| `info-layers`               | 多层带色块的信息层级（A+ 分层 / 内容层级）                                                                                                                                                                                                                   | `items[{layer, name, desc, color?}]`                                                                                                                              |
...
| `keyword-arch`（关键词架构 · SIF） | 流量词反查：核心指标条 + 流量词表（自然/广告排名、周搜索量、流量占比、展示位/标记）。数据源 `keyword_arch.json`（阶段②.8）。建议置于「三-3」                                                                                                                                                      | `metrics[{label,value,hint?,status?}]`、`keywords[{kw,kw_zh?,nat_rank?,ad_rank?,search_vol?,traffic_share_pct?,positions[]?,markers[]?}]`、`note?`                  |
| `traffic-entry`（流量入口 · SIF） | ASIN 流量来源构成：自然 vs SP/SB/SBV/推荐位 的 100% 曝光占比条 + 周期新进/退出词。数据源 `traffic_source.json`（阶段②.8）。建议置于「三-4」                                                                                                                                         | `composition[{label,ratio,color?}]`（ratio 0–1）、`metrics[]?`、`period{total?,in?,out?,prev?}?`、`note?`                                                              |
...
| `w5h2-grid`（V4 新增）          | 5W2H 购买行为分析：Who/What/When/Where/Why/How/How much 七张卡，回答竞品"卖给谁、卖什么、何时何地、为何买、怎么决策、花多少"                                                                                                                                                       | `items[{key, zh?, q?, finding, evidence?, accent?}]`（key 用英文 Who/What/When/Where/Why/How/How much，渲染器据此自动配色）                                                      |
...
```json
{
  ...
  "sections": [
    ...
    { "type": "step-ladder",    "title": "📌 三-1 · 卖点排序 · 三层递进",            "items": ["..."] },
    { "type": "info-layers",    "title": "🪜 三-2 · 信息层级 · A+ 分层拆解",         "items": ["..."] },
...
详见 `.cursor/skills/amazon-multi-asin-visual-synthesizer/SKILL.md`。
...
Step 3：FOR each ASIN: 跑阶段 ③④⑤ 产出单品三件套 + 跑 validate_report_consistency.py
        全部通过校验后才能进入 Step 4
Step 4：⚠️ 强制 AskQuestion 交接询问（三选一）
        ┌─────────────────────────────────────────────────┐
...
**Step 4 询问的标准实现**：必须使用 `AskQuestion` 工具的 Q&A 选项卡（不得只用文字段落追问）。题目示例 JSON：
...
- ❌ **以 token / 时间预算为由擅自降级**：必须先 `AskQuestion` 与用户协商批次方案，**而不是直接给口头总结**
- ❌ **跳过 §5.4 强制询问钩子**：N 份单品三件套出完，不能直接给最终结语就跑路；必须 `AskQuestion` 问 A/B/C 三选一（即使预期用户会选 C 也必须问）
...
- [ ] 是否在 N 个单品三件套都通过校验后，按 §5.4 强制使用 `AskQuestion` 询问用户后续动作（A 补充 / B 对比总结 / C 结束）？
...
- **多 URL 流程末尾强制 `AskQuestion`** 让用户在 "A 补充内容 / B 对比总结 / C 结束" 三选一
...
> V2.4 起所有脚本由 `工具/amazon_competitor_analyzer/` 与 `工具/amazon_image_extractor/` 迁移到对应 Skill 的 `scripts/` 子目录，遵循 Skill 自包含原则（详见 `.cursor/rules/skill-file-structure.mdc`）。

- `.cursor/skills/amazon-image-extractor/scripts/extract_and_stitch.py`（素材采集，被本 Skill 调用 · 归属 `amazon-image-extractor` Skill）
- `.cursor/skills/amazon-competitor-analyzer/scripts/extract_metadata.py`（元数据抽取，本 Skill 专属配套）
- `.cursor/skills/linkfox-amazon-reviews/scripts/amazon_us_reviews.py` / `amazon_reviews.py`（评论自动抓取来源 · 阶段 ②.4 · 用户只提供 URL 且无评论文件时调用；美国站走 US 脚本，非美国站走 domainCode 脚本）
- `.cursor/skills/linkfox-amazon-reviews/scripts/response_io.py`（大响应落盘工具 · 阶段 ②.4 推荐调用方式；落盘后的 `reviews_linkfox_raw*.json` 作为后续评论数据路径）
- `.cursor/skills/amazon-competitor-analyzer/scripts/review_core.py`（评论分析共享核心 · 被 `aggregate_reviews_json.py` 与 `build_review_dashboard.py` import：归一化 .json/.xlsx 以及 LinkFox `data[]` 原始 JSON、数据驱动关键词抽取、各维度聚合）
- `.cursor/skills/amazon-competitor-analyzer/scripts/aggregate_reviews_json.py`（评论聚合 · 阶段 ②.5 · 数据驱动 · 支持 .json/.xlsx/LinkFox raw JSON · **默认只分析目标 ASIN（从 metadata.json 解析），可加 --all-variants 看全系列**）
- `.cursor/skills/amazon-competitor-analyzer/scripts/build_review_dashboard.py`（交互式评论看板生成 · 阶段 ②.6 · 用户评论文件或 LinkFox 评论样本均可产出第 4 件套 `review_dashboard.html` + 外链 `reviews_data.js` · 内联 `assets/echarts.min.js` 离线自包含 · 支持 `--aspects` / `--keywords`）
- `.cursor/skills/amazon-competitor-analyzer/scripts/extract_review_evidence.py`（按优缺点逐条提取全样本评论证据 · 复用 `review_core` · 产出 `review_evidence.json` · 报告的优缺点综合**必须**基于它，禁止抽样拍脑袋）
- `.cursor/skills/amazon-competitor-analyzer/scripts/fetch_keyword_traffic.py`（阶段②.8 · 调 `linkfox-sif-asin-keywords` + `linkfox-sif-asin-summary` 归一化产出 `keyword_arch.json` + `traffic_source.json`（+多URL `kw_traffic_index.json`）· 喂报告「三-3 关键词架构 / 三-4 流量入口」· 需 `LINKFOXAGENT_API_KEY`）
- `.cursor/skills/amazon-competitor-analyzer/assets/echarts.min.js`（本地 ECharts 库 · 被 `build_review_dashboard.py` 内联进看板 HTML）
- `.cursor/skills/amazon-competitor-analyzer/scripts/render_report_html.py`（**单品**报告渲染脚本 · 含左侧套图栏；**不得**用于横向综合可视化沉淀）
- `.cursor/skills/amazon-competitor-analyzer/scripts/validate_report_consistency.py`（一致性校验）
