...
- 入口脚本：`.cursor/skills/brand-asset-sheet/scripts/generate_brand_asset_sheet.py`
...
| 参数               | 必填  | 说明                                                                                                                                                              |
| ---------------- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--project-root` | 否   | 项目根目录绝对路径。**通常不需要传**，脚本默认从自身 `__file__` 向上推5层自动推算（`scripts/` → `brand-asset-sheet/` → `skills/` → `.cursor/` → 项目根）。仅在通过 `exec()` 等特殊方式调用导致 `__file__` 失效时才手动指定 |
