...
`amazon-competitor-analyzer` 跑完所有单品三件套后，必须 `AskQuestion` 询问用户后续动作；当用户选择 **"对比总结 / 综合分析 / 可视化沉淀 / 横向沉淀"** 分支时，本 Skill 被自动调度。
...
- `.cursor/skills/amazon-multi-asin-visual-synthesizer/scripts/build_viz_dashboard.py`（**本 Skill 唯一渲染器** · 数据契约 → 自包含 HTML）
- `.cursor/skills/amazon-multi-asin-visual-synthesizer/templates/viz_data_template.json`（数据契约模板兼可运行样例）
- `.cursor/skills/amazon-multi-asin-visual-synthesizer/assets/echarts.min.js`（本地 ECharts 库 · 被渲染脚本内联进 HTML）
